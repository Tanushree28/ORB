...
class StrategyVisualizer:
    ...
    def plot_performance_heatmap(self, trades_df: pd.DataFrame, save_path: str = None):
        ...
        trades_df['day_of_week'] = pd.to_datetime(trades_df['entry_time'], utc=True).dt.dayofweek
    ...
    def create_html_report(self, summary_df: pd.DataFrame, trades_df: pd.DataFrame,
        ...
        if metrics:
            ...
            for label, value in key_metrics:
                color_class = 'positive' if 'drawdown' not in label.lower() and float(value.replace('$', '').replace('%', '')) > 0 else ''
                metrics_boxes += f"""
        ...
        summary_table = summary_df.to_html(index=False, classes='table') if not summary_df.empty else "<p>No data available</p>"
        
        # Get last 20 trades
        recent_trades = trades_df.tail(20) if not trades_df.empty else pd.DataFrame()
        if not recent_trades.empty:
            recent_trades = recent_trades[['symbol', 'type', 'entry_time', 'entry_price', 
